...
class Sphinx4ResultListener
    ...
    public void newResult(final Result result) {
        ...
        if (res.numTokens() == 0) {
            res.setResultState(jsapi2.javax.speech.src.javax.speech.recognition.Result.REJECTED);

            final ResultEvent event =
                    new ResultEvent(res, ResultEvent.RESULT_REJECTED);
            recognizer.fireResultRejected(event);
        } else {
            res.setResultState(jsapi2.javax.speech.src.javax.speech.recognition.Result.ACCEPTED);
